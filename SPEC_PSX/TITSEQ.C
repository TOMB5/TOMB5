...
char titseqData[] __attribute__((section(".data"))) =
{
	0,
	0,
};
...
int TitleOptions(int Name)
{
	...
	else if (Chris_Menu > 1)
	{
		...
		if (Chris_Menu == 2)
		{
			...
			{
				//loc_9A4
				if ((RawEdge & 0x40) && titseqData[0] < Gameflow->nLevels - 2)
				{
					SoundEffect(SFX_MENU_SELECT, NULL, 2);
					++titseqData[0];
				}
			...
			if (s1 < Gameflow->nLevels - 1)
			{
				...
				else
				{
					...
					if (titseqData[0] == s1)
					{
					 ...
					 //t0 = gfStringOffset
					 //a3 = gfStringWad
					 //a3 = &gfStringWad[gfStringOffset[gfLevelNames[s0]]];
					 ///@TODO tomorrow
				}
				//loc_B94
			}
	...
	else if ((RawEdge & 0x40) && CanLoad + 1 < titseqData[0])
	{
		//loc_7C0
		SoundEffect(SFX_MENU_SELECT, NULL, 2);
		++titseqData[0];
