...
void CreateMonoScreen()//5F450(<), 60130(<) (F) (*) (D)
{
	...
	setRECT16(&r, 0, db.draw[db.current_buffer ^ 1].clip.y, 256, 128);
	...
	setRECT16(&r, 832, 256, 64, 256);

	StoreImage(&r, db.poly_buffer[0] + 9746);
	LoadImagePSX(&r, db.poly_buffer[0] + 17938);

	r.x += 64;

	StoreImage(&r, db.poly_buffer[1] + 9746);
	LoadImagePSX(&r, db.poly_buffer[1] + 17938);
