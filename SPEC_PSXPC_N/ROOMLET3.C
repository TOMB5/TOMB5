...
char* SubPolyGT3RL3(int* t0, int* t1, int s1)
{
    t0 = SubPolyGTLoopRL3(3, t0, t1, s1);

    return NULL;
}

void InitSubdivisionRL3(int* s1, int t1, int s4, int fp, int t5, int t2, int s5, int gp, int t6, int t3, int s6, int s3, int t7, int s7)
{
    s1[186] = t1;
    ((short*)s1)[374] = s4;
    t1 = fp << 8;
        ...
        if (ClipXYRL3(t1, t2, t3, t4) == 0)
        {
            ...
            if (t5 < 0x9E0)
            {
                ...
                if (at != 0)
                {
                    ...
                    s3 = 0;
                    a3 = (int)SubPolyGT3RL3((int*)TriVertTableRL3[0], (int*)&s1[201], (int)s1);
