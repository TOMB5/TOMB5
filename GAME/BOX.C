...
void DropBaddyPickups(ITEM_INFO* item)//259BC(<), 25BC8(<)
{
	short pickup_number;
	short room_number;
	ITEM_INFO* pickup;
...
void CreatureYRot(PHD_3DPOS* srcpos, short angle, short angadd)//25738(<), ?
{
	if (angadd < angle)
	{
		srcpos->y_rot += angadd;
...
short SameZone(creature_info* creature, ITEM_INFO* target_item)//255F8(<), ?
{
	room_info* r;
	short* zone;
	ITEM_INFO* item;
...
short AIGuard(creature_info* creature)//24DF0(<), ?
{
	int random;
...
void CreatureKill(ITEM_INFO* item, int kill_anim, int kill_state, short lara_anim)
{
...
short CreatureEffectT(ITEM_INFO* item, BITE_INFO* bite, short damage, short angle, short* generate)
{
	S_Warn("[CreatureEffectT] - Unimplemented!\n");
	return 0;
}

short CreatureEffect(ITEM_INFO* item, BITE_INFO* bite, short* generate)
{
...
void CreatureTilt(ITEM_INFO* item, short angle)//24418(<), 24624(<) (F)
{
...
int BadFloor(long x, long y, long z, long box_height, long next_height, int room_number, lot_info* LOT)
{
...
enum target_type CalculateTarget(PHD_VECTOR* target, ITEM_INFO* item, lot_info* LOT)
{
...
int ValidBox(ITEM_INFO* item, short zone_number, short box_number)//222A4(<), ?
{
	box_info* box;
...
int EscapeBox(ITEM_INFO* item, ITEM_INFO* enemy, short box_number)//221C4(<), ?
{
	box_info *box; // $a0
	long x; // $a3
	long z; // $a2
...
void TargetBox(lot_info* LOT, short box_number)//220F4(<), ?
{
	box_info* box;
...
int UpdateLOT(lot_info* LOT, int expansion)//22034(<), ?
{
	box_node* expand;
